...
class PathOptimizer:
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        d_n = np.array(d)
        s_n = np.array(s)
        # print("shape ----", a_n.shape, b_n.shape, c_n.shape, d_n.shape, s_n.shape)
        # print(type(a), type(b), type(c), type(d))
        thetas = np.multiply(a_n,s_n) + np.multiply(np.power(s_n,2),np.divide(b_n,2)) + np.multiply(np.power(s_n,3),np.divide(c_n,3)) + np.multiply(np.power(s_n,4),np.divide(d_n,4))
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        # y_cos = np.cos(np.array(t_points))
